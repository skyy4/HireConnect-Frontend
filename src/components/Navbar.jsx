...
export default function Navbar() {
  ...
  const links = useMemo(
    () => NAV_LINKS_BY_ROLE[role] || NAV_LINKS_BY_ROLE.GUEST,
    [role],
  );
  ...
  // Load notifications (skip for ADMIN — admins have their own dashboard)
  const showNotifBell = user?.userId && role !== "ADMIN";
  useEffect(() => {
        ...
        <div className="navbar-actions">
          ...
          {user?.role && (
            <span
              ...
              title={`Logged in as ${ROLE_LABELS[role]}`}
            >
              {ROLE_LABELS[role]}
            </span>
          )}
          ...
          {showMessaging && (
            <Link
              to="/messages"
              className={`nb-icon-btn${isLinkActive(location.pathname, { to: "/messages" }) ? " nb-icon-btn--active" : ""}`}
              aria-label="Messages"
            >
              <IconMessage />
            </Link>
          ...
          {user ? (
            <div className="nb-user-menu">
              <div className="nb-avatar" title={user.name || user.email}>
                {initials}
              </div>
              <button
        ...
        <nav className="mobile-menu" aria-label="Mobile navigation">
          <div className="mobile-menu-section">
            ...
            {showMessaging && (
              <Link
                to="/messages"
                className={`mobile-nav-link${isLinkActive(location.pathname, { to: "/messages" }) ? " active" : ""}`}
              >
                💬 Messages
              </Link>
            )}
            {showNotifBell && (
              <Link
                to="/notifications"
                className={`mobile-nav-link${isLinkActive(location.pathname, { to: "/notifications" }) ? " active" : ""}`}
              >
                🔔 Notifications{" "}
                {unreadCount > 0 && (
          ...
          {user && (
            <div className="mobile-menu-footer">
              <button
                className="mobile-nav-link mobile-nav-button mobile-logout"
                onClick={handleLogout}
              >
                <IconLogout /> Logout
              </button>
