...
export default function Profile() {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      if (!isRecruiter && user?.email) {
        try {
          loadedProfile = await ensureCandidateProfile(user);
          setProfile(loadedProfile);
          setProfileId(loadedProfile.profileId);
          setForm(loadedProfile);
          setLoadError("");
    ...
    if (!isRecruiter && loadedProfile?.profileId) {
      try {
        ...
        /* Not parsed yet — silently ignore */
      }
    }
  ...
  const handleTopup = async (e) => {
    ...
    try {
      ...
      setToast("Failed to recharge wallet.");
    }
  };

  const handleParseResume = async () => {
    if (!profile?.resumeUrl) {
      setToast("Please upload or add a Resume URL first!");
      return;
    }
    setParsing(true);
    try {
  ...
  const handleResumeUpload = async () => {
    ...
    if (resumeFile.type !== "application/pdf") {
      setToast("Only PDF resumes are supported.");
      return;
    }
    ...
    try {
      ...
      if (uploadedParsed?.resumeUrl) {
        setProfile((prev) => ({
          ...(prev || {}),
          resumeUrl: uploadedParsed.resumeUrl,
        }));
  ...
  const handleInviteTeam = async (e) => {
    e.preventDefault();
    try {
      await inviteTeamMember(profileId, {
        email: inviteEmail,
        teamRole: inviteRole,
      });
    ...
    <div className="page-wrapper">
      ...
      <div className="page-content">
        <div className="page-header">
          <div>
            ...
            <p>
              Manage your {isRecruiter ? "company" : "candidate"} profile
              information
            </p>
          </div>
          {!editing && (
        ...
        <div className="profile-layout">
          ...
          <div className="profile-sidebar">
            ...
            <span
              className={`role-badge ${isRecruiter ? "recruiter" : "candidate"}`}
            >
              ...
              <div className="profile-form">
                ...
                {!isRecruiter && (
                  <div className="form-section">
                    ...
                    <div className="form-group">
                      <label className="form-label">
                        Resume URL (optional — or upload below)
                      </label>
                      <input
                        type="url"
                        className="form-input"
                    ...
                    <div className="profile-section profile-team-section">
                      ...
                      <div className="list-card profile-team-card">
                              ...
                              >
                                <div>
                                  ...
                                  <div className="profile-team-meta">
                                    {m.email} · Role: {m.teamRole || m.role} ·
                                    Status: {m.status}
                                  </div>
                                </div>
                                {m.status !== "REVOKED" && (
                        ...
                        <form
                          ...
                          className="profile-team-invite"
                        >
                          <div className="form-group profile-team-email-group">
                            <label className="form-label profile-team-label">
                              Email Address
                            </label>
                            <input
                              type="email"
                              required
                              className="form-input profile-team-input"
                              value={inviteEmail}
                          ...
                          <button
                            className="btn-primary profile-team-invite-btn"
                            type="submit"
                          >
                            Invite
                          </button>
      ...
      >
        <p className="muted profile-modal-copy">
          Remove <strong>{revokeTarget?.name || "this member"}</strong> from
          your recruiter team?
        </p>
        <div className="btn-row profile-modal-actions">
          <button
            ...
            disabled={revokingMember}
          >
            Cancel
          </button>
          <button
            className="btn-danger"
            onClick={confirmRevokeTeam}
            disabled={revokingMember}
          >
