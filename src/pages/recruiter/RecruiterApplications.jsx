...
export default function RecruiterApplications() {
  ...
  const fetchProfiles = useCallback(async (apps) => {
    ...
    const entries = await Promise.allSettled(
      uniqueIds.map((uid) =>
        getCandidateByUserId(uid).then((r) => [uid, r.data]),
      ),
    );
    const map = {};
    entries.forEach((r) => {
  ...
  const handleStatusUpdate = async (appId, status) => {
    try {
      ...
      setApplications((prev) =>
        prev.map((a) => (a.applicationId === appId ? res.data : a)),
      );
            ...
            <div className="split-main">
              {!selectedJob ? (
                <EmptyState
                  icon="👈"
                  title="Select a job"
                  message="Choose a job from the left to view its applications."
                />
              ) : loadingApps ? (
                <LoadingSpinner />
              ) : (
                <>
                  ...
                  {filtered.length === 0 ? (
                    <EmptyState
                      ...
                      message={`No applications with status: ${filter.replace(/_/g, " ")}`}
                    />
                  ) : (
                          ...
                          <div className="app-card" key={app.applicationId}>
                            <div className="app-card-left">
                              ...
                              <div>
                                ...
                                {(app.resumeUrl ||
                                  profileMap[app.candidateId]?.resumeUrl) && (
                                  <a
                                    href={getResumeFullUrl(
                                      app.resumeUrl ||
                                        profileMap[app.candidateId]?.resumeUrl,
                                    )}
                                    target="_blank"
                                ...
                                {app.status === "SHORTLISTED" && (
                                  <>
                                    <button
                                      ...
                                      onClick={() =>
                                        navigate(
                                          `/recruiter/jobs/${selectedJob}/applications`,
                                        )
                                      }
                                    >
                                      Schedule Interview
                                    ...
                                    <button
                                      ...
                                      onClick={() =>
                                        navigate(
                                          `/messages?candidateId=${app.candidateId}&applicationId=${app.applicationId}`,
                                        )
                                      }
                                    >
                                      Message
